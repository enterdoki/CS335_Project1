...
Description : The implementation of AvlTree.h.
Usage :
Build with : Makefile
Modifications:
    Majority of the implementation is provided by my instructor, Professor Stewart Weiss.
    The remove function was taken from his notes which is obtained from the textbook:
        Data Structures & Algorithm Analysis in C++, 4th Edition. Mark Allen Weiss. Pearson,
...
void AvlTree<Comparable>::remove( const Comparable & x, AvlNode<Comparable> * & t ) const{
    if(t == NULL)   // can't delete from an empty tree
        return;
    if(x < t->element) {    //delete from left subtree
        remove(x, t->left);
    ...
    else if(t->element < x) {   //delete from the right subtree
        remove(x, t->right);
